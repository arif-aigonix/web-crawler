class URLCrawler {
    ...
    flushStreamBuffer() {
        ...
        this.streamBuffer.forEach((item) => {
            ...
            const shouldShow =
                this.currentDepthFilter === null ||
                this.currentDepthFilter === item.depth;
        ...
        if (this.currentDepthFilter !== null) {
            const visibleItems = Array.from(
                this.streamContainer.children,
            ).filter(
                (item) =>
                    parseInt(item.getAttribute("data-depth")) ===
                    this.currentDepthFilter,
            );
            console.log("Current visible items:", visibleItems.length);

            // Remove oldest items of the current depth if we're over the limit
            while (
                visibleItems.length + this.streamBuffer.length >
                this.maxStreamItems
            ) {
            ...
            while (
                this.streamContainer.children.length +
                    this.streamBuffer.length >
                this.maxStreamItems
            ) {
                this.streamContainer.removeChild(
                    this.streamContainer.firstChild,
                );
    ...
    clearStream() {
        ...
        const depthStatsSection = document.querySelector(
            ".depth-stats-section",
        );
    ...
    async start() {
        ...
        try {
            ...
            this.usePuppeteer =
                document.getElementById("useDynamicCrawling").checked;
            ...
            this.maxDepth = maxDepthInput.value
                ? parseInt(maxDepthInput.value)
                : Infinity;
    ...
    setupEventListeners() {
        ...
        document
            .getElementById("resetDepthFilter")
            .addEventListener("click", () => {
        ...
        document.getElementById("clearStream").addEventListener("click", () => {
            if (this.processing) {
                if (
                    confirm(
                        "Crawler is still running. Clearing the stream will only hide existing results but new URLs will continue to appear. Do you want to proceed?",
                    )
                ) {
                    this.clearStream();
                }
        ...
        try {
            ...
            const response = await fetch("/fetch", {
                ...
                body: JSON.stringify({
                    url: url,
                    respectRobots:
                        document.getElementById("respectRobotsTxt").checked,
    ...
    async extractUrls(html, sourceUrl) {
        ...
        try {
            ...
            console.log(
                `Parsing HTML from ${sourceUrl}, length: ${html.length}`,
            );
            ...
            const elements = doc.querySelectorAll(
                'a[href], link[href][rel="alternate"], [onclick], [data-href]',
            );
            ...
            console.log(
                `Found ${urlArray.length} unique URLs on page ${sourceUrl}`,
            );
            return urlArray;
        } catch (error) {
    ...
    updateDepthStatsUI() {
        ...
        const sortedDepths = Array.from(this.depthStats.keys()).sort(
            (a, b) => a - b,
        );
        ...
        sortedDepths.forEach((depth) => {
            ...
            depthButton.className =
                "depth-stat-item" +
                (this.currentDepthFilter === depth ? " active" : "");
    ...
    filterStreamByDepth(depth) {
        ...
        const streamItems =
            this.streamContainer.getElementsByClassName("stream-item");
        ...
        if (visibleCount < totalAtDepth) {
            ...
            try {
                ...
                if (urlData) {
                    ...
                    Array.from(streamItems).forEach((item) => {
                        if (
                            parseInt(item.getAttribute("data-depth")) === depth
                        ) {
                            item.remove();
                            removedCount++;
                        }
    ...
    updateUI() {
        ...
        document.getElementById("acceptedCount").textContent =
            this.acceptedCount;
        document.getElementById("status").textContent = this.processing
            ? "Crawling..."
            : "Ready";
    ...
    cleanup() {
        ...
        console.log(
            "Crawler memory cleaned up. Keeping",
            acceptedUrls.size,
            "accepted URLs",
        );
    }
